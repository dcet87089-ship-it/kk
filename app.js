...
/* ==========================================================
   Silent Visitor Tracker (Discord Webhook Integration)
   Tracks platform (Instagram, Facebook, etc.), device,
   location, and timestamp completely silently!
   ========================================================== */
function initSilentVisitorTracker() {
  const WEBHOOK_URL = 'https://discord.com/api/webhooks/1551578766235537418/yVzjrHpCggo9zh3qMzy_7fHqvwKZirtZ5NtNY89Vye_j-4xveL5MdDP9JvF0POaUR5x6';

  // Prevent spamming Discord if the visitor reloads within the same session
  const lastTracked = sessionStorage.getItem('sky_visit_tracked');
  if (lastTracked) return;
  sessionStorage.setItem('sky_visit_tracked', Date.now().toString());

  // Determine if new or returning visitor
  const isReturning = localStorage.getItem('sky_visitor_id');
  if (!isReturning) {
    localStorage.setItem('sky_visitor_id', 'v_' + Math.random().toString(36).substring(2, 9));
  }

  // Detect Platform Source (Instagram, Facebook, TikTok, LINE, Direct)
  const currentUrl = window.location.href;
  const searchParams = new URLSearchParams(window.location.search);
  const referrer = document.referrer.toLowerCase();

  let platform = '🌐 เข้าชมโดยตรง / Direct Link';
  let platformColor = 3717112; // Sky Blue
  let platformIcon = '🌐';

  if (searchParams.get('utm_source') === 'ig' || referrer.includes('instagram.com') || (searchParams.has('fbclid') && currentUrl.includes('instagram'))) {
    platform = '📸 Instagram (Link in Bio / Story)';
    platformColor = 14431557; // Magenta
    platformIcon = '📸';
  } else if (searchParams.has('fbclid') || searchParams.get('utm_source') === 'fb' || referrer.includes('facebook.com')) {
    platform = '👤 Facebook (Post / Profile / Share)';
    platformColor = 1603570; // Blue
    platformIcon = '👤';
  } else if (searchParams.get('utm_source') === 'tiktok' || referrer.includes('tiktok.com')) {
    platform = '🎵 TikTok (Link in Bio)';
    platformColor = 2282222; // Neon Cyan
    platformIcon = '🎵';
  } else if (referrer.includes('line.me')) {
    platform = '💬 LINE (แชท / ไทม์ไลน์)';
    platformColor = 4443477; // Green
    platformIcon = '💬';
  }

  // Detect Device & In-App Browser
  const ua = navigator.userAgent || '';
  let deviceType = '💻 คอมพิวเตอร์ (PC / Mac)';
  if (/iPad|iPhone|iPod/.test(ua) && !window.MSStream) {
    deviceType = '📱 iPhone / iPad (iOS)';
  } else if (/Android/.test(ua)) {
    deviceType = '📱 มือถือ Android';
  }

  let inAppBrowser = '';
  if (/Instagram/i.test(ua)) {
    inAppBrowser = ' [Instagram In-App Browser]';
  } else if (/FBAN|FBAV/i.test(ua)) {
    inAppBrowser = ' [Facebook In-App Browser]';
  } else if (/Line/i.test(ua)) {
    inAppBrowser = ' [LINE In-App Browser]';
  }

  const screenSize = `${window.screen.width}x${window.screen.height}`;
  const now = new Date();
  const timeFormatted = now.toLocaleString('th-TH', { timeZone: 'Asia/Bangkok' });

  // Silent asynchronous Geo-location lookup
  async function track() {
    let locationStr = '📍 ประเทศไทย (กำลังโหลดข้อมูล)';
    let ispStr = '';
    ...
    try {
      ...
      if (geoRes.ok) {
        ...
        if (geo.success) {
          ...
          locationStr = `📍 ${city ? city + ', ' : ''}${region ? region + ', ' : ''}${country}`;
          ispStr = isp ? ` (${isp})` : '';
        }
      }
    } catch (e) {
      locationStr = '📍 ประเทศไทย (โดยประมาณ)';
    }

    const visitorStatus = isReturning ? '🔄 คนดูเดิมกลับมาเปิดซ้ำ' : '✨ คนดูใหม่ (New Visitor)';

    const payload = {
      username: 'Sky Bio Tracker 👁️',
      embeds: [
        {
          title: `🔔 มีคนกดเข้ามาดูเว็บของคุณ!`,
          description: `**สถานะ:** ${visitorStatus}`,
          color: platformColor,
          fields: [
            {
              name: `${platformIcon} แพลตฟอร์มที่มา`,
              value: `**${platform}**`,
              inline: true
            },
            {
              name: '📱 อุปกรณ์',
              value: `${deviceType}${inAppBrowser}\n\`ขนาดจอ: ${screenSize}\``,
              inline: true
            },
            {
              name: 'พิกัด / เครือข่าย',
              value: `${locationStr}${ispStr}`,
              inline: false
            },
            {
              name: '⏰ เวลาที่เปิดดู',
          ...
          footer: {
            text: 'Chatkawee (Sky) • Official Link Tracker'
          },
